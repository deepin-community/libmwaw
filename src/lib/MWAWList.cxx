...
////////////////////////////////////////////////////////////
void MWAWListLevel::addTo(librevenge::RVNGPropertyList &propList, MWAWFontManagerPtr fontManager) const
{
  ...
  if (m_spanId>=0 && fontManager) {
    MWAWFont font;
    if (fontManager->getFont(m_spanId, font))
      font.addToListLevel(propList, fontManager->getFontConverter());
  }
}
...
std::ostream &operator<<(std::ostream &o, MWAWListLevel const &level)
{
  ...
  if (!level.m_prefix.empty()) o << ", prefix='" << level.m_prefix.cstr()<<"'";
  if (!level.m_suffix.empty()) o << ", suffix='" << level.m_suffix.cstr()<<"'";
  ...
  if (level.m_spanId>=0) o << ", span[id]='" << level.m_spanId << "'";
  o << "]";
...
bool MWAWList::addTo(int level, librevenge::RVNGPropertyList &pList, MWAWFontManagerPtr fontManager) const
{
  ...
  m_levels[size_t(level-1)].addTo(pList, fontManager);
  return true;
}
...
////////////////////////////////////////////////////////////
MWAWListManager::MWAWListManager(MWAWFontManagerPtr const &fontManager) : m_fontManager(fontManager), m_listList(), m_sendIdMarkerList()
{
}
...
std::shared_ptr<MWAWList> MWAWListManager::getNewList(std::shared_ptr<MWAWList> actList, int levl, MWAWListLevel const &level)
{
  ...
  res.set(levl, level);
  for (auto &list : m_listList) {
    if (!list.isCompatibleWith(res))
      continue;
    if (list.numLevels() < levl)
      list.set(levl, level);
    std::shared_ptr<MWAWList> copy(new MWAWList(list));
    copy->updateIndicesFrom(res);
    return copy;
  }
  m_listList.push_back(res);
  return std::shared_ptr<MWAWList>(new MWAWList(res));
}
