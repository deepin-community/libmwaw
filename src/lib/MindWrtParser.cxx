...
struct Field {
  //! constructor
  explicit Field(MWAWField::Type type=MWAWField::None)
    : m_type(type)
    , m_pos(0,-1)
    , m_extra("")
  {
...
struct ListProperties {
  ...
  ListProperties()
    : m_startListIndex(1)
    , m_headingStyle(1)
    , m_headingFullSubLevels(true)
    , m_headingListLevels()
    , m_listLevelsRepeatPos(1)
    , m_showFirstLevel(false)
    , m_useHeadingStyle(false)
  {
  }
...
{
  auto numLevels=int(m_headingListLevels.size());
...
struct LineInfo {
  LineInfo()
    ...
    , m_page(-1)
    , m_paragraph()
    , m_specialHeadingInterface(false)
    , m_paragraphSet(false)
    , m_listLevel(0)
    , m_listType(0)
    , m_extra("")
  {
...
void ZoneInfo::updateListId(ListProperties &prop, MWAWListManager &listManager)
{
  ...
  std::shared_ptr<MWAWList> listsList[4];
  int maxLevelSet=0;
  ...
  for (auto &line : m_linesList) {
    ...
    int level = line.m_listLevel+(prop.m_showFirstLevel ? 1 : 0);
    if (level < 0)
      level = 0;
    ...
    auto &actList=listsList[line.m_listType];
    if (!actList) // we have a pb....
      continue;
    for (auto &type : listsList)
      if (type) type->setLevel(level);
...
struct State {
  ...
  State()
    ...
    , m_footerFieldList()
  {
    for (auto &num : m_numLinesByZone) num = 0;
  }
...
void SubDocument::parse(MWAWListenerPtr &listener, libmwaw::SubDocumentType /*type*/)
{
  ...
  auto *parser=dynamic_cast<MindWrtParser *>(m_parser);
  if (!parser) {
...
////////////////////////////////////////////////////////////
MindWrtParser::MindWrtParser(MWAWInputStreamPtr const &input, MWAWRSRCParserPtr const &rsrcParser, MWAWHeader *header)
...
void MindWrtParser::createDocument(librevenge::RVNGTextInterface *documentInterface)
{
  ...
  for (auto const &line : m_state->m_zones[0].m_linesList) {
    if (line.m_type == -2)
      numBreakPage++;
  ...
  for (int i = 1; i <= 2; i++) {
    if (m_state->m_zones[i].m_linesList.empty())
      continue;
...
bool MindWrtParser::createZones()
{
  ...
  auto it = m_state->m_entryMap.find("PrintInfo");
  ...
  for (auto const &info : m_state->m_zones)
    if (info.m_linesList.size())
      return true;
  return false;
}
...
bool MindWrtParser::sendZone(int id)
{
  ...
  auto &zone = m_state->m_zones[id];
  for (size_t i = 0; i < zone.m_linesList.size(); i++) {
    auto &line = zone.m_linesList[i];
    if (i==0) {
...
bool MindWrtParser::readGraphic(MindWrtParserInternal::LineInfo const &line)
{
  ...
  MWAWInputStreamPtr input = getInput();
  libmwaw::DebugStream f;
  auto sz = static_cast<int>(line.m_entry.length());
  if (sz < 10) {
  ...
  MWAWBox2f box(MWAWVec2f(static_cast<float>(dim[1]),static_cast<float>(dim[0])),
                MWAWVec2f(static_cast<float>(dim[3]),static_cast<float>(dim[2])));
  f << "Entries(graphic): bdBox=" << box << ",";

  std::shared_ptr<MWAWPict> pict(MWAWPictData::get(input, sz-8));
  if (!pict) {
    MWAW_DEBUG_MSG(("MindWrtParser::readGraphic: can not read the picture\n"));
    return false;
  }
  MWAWEmbeddedObject picture;
  if (getTextListener() && pict->getBinary(picture)) {
...
bool MindWrtParser::readRuler(MindWrtParserInternal::LineInfo &line)
{
  ...
  if (line.m_entry.length() < 10 || (line.m_entry.length()%2) || !input->checkPosition(line.m_entry.end())) {
...
void MindWrtParser::sendHeaderFooter(bool header)
{
  ...
  int zId=header ? 1 : 2;
  auto const &fieldList= header ? m_state->m_headerFieldList : m_state->m_footerFieldList;
  if (!fieldList.empty()) {
    ...
    std::shared_ptr<MindWrtParserInternal::SubDocument> subDoc
...
void MindWrtParser::sendHeaderFooterFields(bool header)
{
  ...
  auto fieldList=header ? m_state->m_headerFieldList : m_state->m_footerFieldList;
  ...
  for (size_t i=0; i+1 < numFields; i++) {
    for (size_t j=i+1; j < numFields; j++) {
      ...
      if (diffLine > 0 ||
        ...
        continue;
      std::swap(fieldList[i], fieldList[j]);
    }
  }
...
bool MindWrtParser::checkHeader(MWAWHeader *header, bool strict)
{
  *m_state = MindWrtParserInternal::State();
  ...
  f << "Entries(Header):";
  for (auto &num : m_state->m_numLinesByZone) num = static_cast<int>(input->readULong(2));
  ...
  if (val != 1) f << "g2=" << val << ",";
  m_state->m_listProperties.m_startListIndex = static_cast<int>(input->readULong(1))+1;
  ...
  if (strict) {
    ...
    for (int i : m_state->m_numLinesByZone) {
      input->seek(4, librevenge::RVNG_SEEK_CUR);
      if (input->readLong(4) == 32*i)
        continue;
      ok = false;
      break;
    }
...
bool MindWrtParser::readLinesInfo(MWAWEntry &entry)
{
  ...
  libmwaw::DebugStream f;
  auto &textZone = m_state->m_zones[entry.id()];
  ...
  for (int n = 0; n < N; n++) {
    ...
    pos = input->tell();
    f.str("");
    line.m_type = static_cast<int>(input->readLong(1)); // 0, fd, ff
    line.m_height = static_cast<int>(input->readULong(1));
    line.m_y = static_cast<int>(input->readLong(2));
    line.m_page = static_cast<int>(input->readULong(1)); // check me
    long val = input->readLong(2); // -1 or between 5d7-5de
    f << "f0=" << std::hex << val << std::dec << ",";
    for (int i=0; i < 2; i++)
      line.m_flags[i] = static_cast<int>(input->readULong(1));
    auto ptr = long(input->readULong(1));
    line.m_entry.setBegin((ptr << 16) | long(input->readULong(2)));
    line.m_entry.setLength(long(input->readULong(2)));
    line.m_extra=f.str();
    for (int i=2; i < 4; i++)
      line.m_flags[i] = static_cast<int>(input->readULong(1));
    line.m_listLevel = static_cast<int>(input->readLong(2));
    line.m_listType = static_cast<int>(input->readLong(2)); //0: heading, 1: unlabelled 2: paragraph, 3: bullet
    /** 01[45]0 */
  ...
  for (auto &line : textZone.m_linesList) {
...
bool MindWrtParser::readHeadingCustom(MWAWEntry &entry)
{
  ...
  libmwaw::DebugStream f;
  f << "HeadCust:";
  if (static_cast<int>(input->readULong(2))!= len) {
  ...
  auto debDeplPos = static_cast<int>(input->readLong(2));
  auto debStringPos = static_cast<int>(input->readLong(2));
  ...
  for (int i = 0; i < N; i++)
    listPos.push_back(static_cast<int>(input->readLong(2)));
  ...
  for (size_t i = 0; i < size_t(N); i++) {
    ...
    for (int s = 0; s < sSz; s++) {
      ...
      if (!seeIndex) {
        ...
        default:
          seeIndex=false;
          unicode = getParserState()->m_fontConverter->unicode(3, static_cast<unsigned char>(c));
          if (unicode==-1)
            libmwaw::appendUnicode(static_cast<unsigned char>(c), level.m_prefix);
          else
      ...
      unicode = getParserState()->m_fontConverter->unicode(3, static_cast<unsigned char>(c));
      if (unicode==-1)
        libmwaw::appendUnicode(static_cast<unsigned char>(c), level.m_suffix);
      else
...
bool MindWrtParser::readHeadingProperties(MWAWEntry &entry)
{
  ...
  if (val) f << "f0=" << val << ",";
  m_state->m_listProperties.m_headingStyle=static_cast<int>(input->readULong(1));
...
bool MindWrtParser::readZone8(MWAWEntry &entry)
{
  ...
  libmwaw::DebugStream f;
  f << "Zone8:";
  auto val = long(input->readULong(1));// fl0=0|1|2c|2d,
  if (val)
...
bool MindWrtParser::readHeadingFields(MWAWEntry &entry)
{
  ...
  for (int i = 0; i < num; i++) {
    ...
    f.str("");
    int dim[2];
    for (int j=0; j < 2; j++) dim[j] = static_cast<int>(input->readLong(j==0 ? 2:4));
...
void MindWrtParser::sendText(std::string const &text, std::vector<MWAWFont> const &fonts, std::vector<int> const &textPos)
{
  ...
  for (size_t c = 0; c < numChar; c++) {
    ...
    auto ch = static_cast<unsigned char>(text[c]);
    switch (ch) {
    case 0x9:
    ...
    case 0xd:
      ...
      break;
    default:
      getTextListener()->insertCharacter(static_cast<unsigned char>(ch));
      break;
    }
  }
}
