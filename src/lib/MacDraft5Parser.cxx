...
struct Layout {
  ...
  {
    if (!id) return std::shared_ptr<Shape>();
...
void Layout::updateRelations()
{
  ...
  for (auto shape : m_shapeList) {
    ...
    for (auto &childId : shape->m_childList) {
      if (idToParentMap.find(childId)!=idToParentMap.end() &&
          idToParentMap.find(childId)->second==id)
        continue;
      MWAW_DEBUG_MSG(("MacDraft5ParserInternal::Layout::updateRelations: %lx is not a child of %lx\n", static_cast<unsigned long>(childId), static_cast<unsigned long>(id)));
      childId=0;
    }
  }
  // check for loop
  ...
  for (auto id : m_rootList) toCheck.push(id);
  ...
  while (true) {
    ...
    for (auto &id : shape.m_childList) {
      if (id==0) continue;
      if (m_idToShapePosMap.find(id)==m_idToShapePosMap.end() ||
          seens.find(m_idToShapePosMap.find(id)->second)!=seens.end()) {
        MWAW_DEBUG_MSG(("MacDraft5ParserInternal::Layout::updateRelations: find loop for child %lx\n", static_cast<unsigned long>(id)));
        id=0;
        continue;
      }
      toCheck.push(m_idToShapePosMap.find(id)->second);
    }
  }
}
...
void SubDocument::parse(MWAWListenerPtr &listener, libmwaw::SubDocumentType)
{
  ...
  auto *parser=dynamic_cast<MacDraft5Parser *>(m_parser);
  if (!parser) {
...
////////////////////////////////////////////////////////////
MacDraft5Parser::MacDraft5Parser(MWAWInputStreamPtr const &input, MWAWRSRCParserPtr const &rsrcParser, MWAWHeader *header)
  ...
  try {
    ...
    if (ok) {
      ...
      if (!m_state->m_imageList.empty()) {
        ...
        for (auto layout : m_state->m_layoutList) {
          if (layout)
            send(*layout);
        }
      }
...
void MacDraft5Parser::createDocument(librevenge::RVNGDrawingInterface *documentInterface)
{
  ...
  if (m_state->m_imageList.empty())
    ...
    for (auto image : m_state->m_imageList) {
      MWAWPageSpan page(ps);
      if (image)
        page.setPageName(image->m_name);
...
bool MacDraft5Parser::createZones()
{
  ...
  if (m_state->m_layoutList.empty()) {
    ...
    while (!input->isEnd()) {
      pos=input->tell();
      std::shared_ptr<MacDraft5ParserInternal::Layout> layout(new MacDraft5ParserInternal::Layout(static_cast<int>(m_state->m_layoutList.size())));
      ...
      long newN=static_cast<int>(input->readULong(4));
    ...
    for (auto layout : m_state->m_layoutList) {
      if (!layout || !layout->m_entry.valid())
        continue;
      if (layout->m_entry.end()>lastPosSeen)
        lastPosSeen=layout->m_entry.end();
      readLayout(*layout);
    }
  }
...
bool MacDraft5Parser::readDocHeader()
{
  ...
  int nPages[2];
  for (auto &n : nPages) n=static_cast<int>(input->readLong(2));
  ...
  if (fVal<0 || fVal>0)
    f << "d3=" << fVal << ",";
  val=static_cast<int>(input->readLong(2)); // 3|e|d
  if (val) f << "f13=" << val << ",";
  ...
  if (coord[0]||coord[1]) f << "dim=" << MWAWVec2i(coord[1],coord[0]) << ",";
...
bool MacDraft5Parser::readLibraryFooter()
{
  ...
  for (long i=0; i<N; ++i) {
    pos=input->tell();
    f.str("");
    std::shared_ptr<MacDraft5ParserInternal::Image> image(new MacDraft5ParserInternal::Image);
    ...
    for (int c=0; c<sSz; ++c) {
      ...
      if (unicode==-1)
        ...
        libmwaw::appendUnicode(static_cast<uint32_t>(unicode), image->m_name);
    }
    input->seek(pos+30, librevenge::RVNG_SEEK_SET);
    val=static_cast<int>(input->readLong(2)); // constant in a zone: 6|d
    if (val) f << "f0=" << val << ",";
    val=static_cast<int>(input->readLong(2)); // 0|1
    if (val) f << "f1=" << val << ",";
    image->m_extra=f.str();
...
bool MacDraft5Parser::readObject(MacDraft5ParserInternal::Layout &layout)
{
  ...
  libmwaw::DebugStream f;
  f << "Entries(Object):";
  std::shared_ptr<MacDraft5ParserInternal::Shape> shape(new MacDraft5ParserInternal::Shape);
  ...
  auto val=static_cast<int>(input->readULong(1)); // 0|80
  if (val) f << "type[h]=" << std::hex << val << std::dec << ",";
  shape->m_fileType=static_cast<int>(input->readULong(1)); //[0-6][0-b]
  ...
  int rotation = -static_cast<int>(input->readLong(2));
  style.m_rotate=float(rotation);
  if (rotation) f << "rotate=" << rotation << ",";
  val=static_cast<int>(input->readLong(2)); // 0|2|13b3|2222
  if (val) f << "f0=" << val << ",";
  ...
  if (val!=2) // alway 2?
    f << "f2=" << val << ",";
  auto lineType=static_cast<int>(input->readULong(1));
  // FIXME: storeme...
  auto dashId=static_cast<int>(input->readULong(1));
  if (dashId&0x80) {
    switch ((dashId>>4)&0x7) {
    case 0:
      f << "hairline,";
    ...
    f << "##dash[high]=" << (dashId>>4) << ",";
  }
  auto arrowId=static_cast<int>(input->readULong(1)); // low|end

  auto colId=static_cast<int>(input->readULong(2)); // 0-78
  f << m_styleManager->updateLineStyle(lineType, colId, (dashId&0xf), style);

  float coord[2];
  for (auto &c : coord) c=float(input->readLong(4))/65536.f;
  shape->m_origin=MWAWVec2f(coord[1],coord[0]);
  f << "pos=" << shape->m_origin << ",";

  val=static_cast<int>(input->readULong(2)); // [02][0134][04][01]
  if (val&1) f << "selected,";
  if (val&8) f << "lock,";
  val &= 0xFFF6;
  ...
  case 0x1f: { // spline2
    ...
    if (val) // alway 2?
      f << "f3=" << val << ",";
    auto surfType=static_cast<int>(input->readULong(1));
    colId=static_cast<int>(input->readULong(2)); // 0-78
    ...
    if (shape->m_fileType==3 || shape->m_fileType==4) {
      ...
      if (fSz<48+10*static_cast<int>(nbPts)) {
      ...
      val=static_cast<int>(input->readULong(1)); // 0-2
      ...
      val=static_cast<int>(input->readULong(1)); // 0-1
      if (val) f << "orig[pos]=" << val << ",";
      val=static_cast<int>(input->readULong(1)); // always 0
      if (val) f << "g1=" << val << ",";
      ...
      if (val) f << "g0=" << val << ",";
      nbPts=size_t(input->readULong(2));
      if (fSz<48+30*static_cast<int>(nbPts)+10) {
        MWAW_DEBUG_MSG(("MacDraft5Parser::readObject: the number of points seems bad\n"));
        f << "##N=" << nbPts << ",";
        break;
      }
      ...
      for (size_t i=0; i<=nbPts; ++i) {
        ...
        for (int j=0; j<nCoord; ++j) {
          ...
          points[j]=MWAWVec2f(coord[1],coord[0]);
          f << points[j];
    ...
    for (size_t i=0; i<nbPts; ++i) {
      ...
      for (auto &c : coord) c=float(input->readLong(4))/65536.f;
      listPts[i]=MWAWVec2f(coord[1],coord[0]);
      f << listPts[i];
      if (!f2.str().empty())
        f << ":[" << f2.str() << "],";
      else
    ...
    if (shape->m_fileType==0xb) {
      val=static_cast<int>(input->readULong(1)); // 1-3
      if (val!=1) f << "orig[pos]=" << val << ",";
      val=static_cast<int>(input->readULong(1)); // always 0
      if (val) f << "h4=" << val << ",";
      val=static_cast<int>(input->readULong(2)); // always 4
      if (val!=4) f << "h5=" << val << ",";
      for (auto &c : coord) c=float(input->readLong(4))/65536.f;
      corner=MWAWVec2f(coord[1]/2.f,coord[0]/2.f);
      f << "corner=" << corner << ",";
    }
    switch (shape->m_fileType) {
    case 0x9:
  ...
  case 0x28: { // text
    ...
    f << "pts=[";
    int n=0;
    for (auto &point : listPts) {
      libmwaw::DebugStream f2;
      val=static_cast<int>(input->readULong(1));
      if (val!=(n++?0:1)) f2 << "h0=" << val << ",";
      val=static_cast<int>(input->readULong(1));
      if (val) f2 << "fl=" << std::hex << val << std::dec << ",";
      for (auto &c : coord) c=float(input->readLong(4))/65536.f;
      point=MWAWVec2f(coord[1],coord[0]);
      f << point;
      if (!f2.str().empty())
        f << ":[" << f2.str() << "],";
      else
    ...
    if (rotation) {
      MWAWBox2f bdbox;
      MWAWVec2f center=0.5f*(listPts[0]+listPts[2]);
      for (int i=0; i<4; ++i) {
  ...
  case 0x30: { // bitmap
    ...
    shape->m_bitmapIdList.push_back(static_cast<int>(input->readULong(2)));
  ...
  case 0x35: { // group
    ...
    auto nChild=static_cast<int>(input->readULong(2));
  ...
  case 0x58: {
    ...
    if (shape->m_fileType==0x57) { // cross quotation, no text
      for (auto &c : coord) c=float(input->readLong(4))/65536.f;
      MWAWVec2f axis(coord[1],coord[0]);
      f << "axis,dim=" << axis << ",";
      rotation=0;
      ...
      shape->m_otherShapeList.push_back(MWAWGraphicShape::line(shape->m_origin-MWAWVec2f(-coord[0],coord[1]),shape->m_origin+MWAWVec2f(-coord[0],coord[1])));
      break;
    }
  ...
  case 0x5e: { // correspondance between surface quotation and parent
    ...
    auto pId=long(input->readULong(4));
    f << "parent[id]=" << std::hex << pId << std::dec << ",";
    if (shape->m_parentId==0) shape->m_parentId=pId;
    shape->m_type=MacDraft5ParserInternal::Shape::Group;
    shape->m_childList.push_back(static_cast<int>(input->readULong(4)));
  ...
  case 0x63: { // surface quotation
    ...
    for (auto &c : coord) c=float(input->readLong(4))/65536.f;
    f << "pos1=" << MWAWVec2f(coord[1],coord[0]) << ",";;
  ...
  case 0x64: { // line para
    ...
    for (size_t i=0; i<2; ++i) {
      for (int j=0; j<3; ++j) { // checkme: original line, line1, line2?
        ...
        for (auto &c : coord) c=float(input->readLong(4))/65536;
        listPts.push_back(MWAWVec2f(coord[1],coord[0]));
        f << listPts.back();
        if (!f2.str().empty()) f << "[" << f2.str() << "]";
        if (j+1==3) f << ",";
        else f << ":";
  ...
  case 0x65: { // poly para
    ...
    if (val) f << "g7=" << val << ",";
    auto nbPts=static_cast<int>(input->readULong(2));
    if (fSz<58+30*nbPts) {
    ...
    for (int p=0; p<3; ++p) {
      ...
      for (int j=0; j<nbPts; ++j) {
        ...
        for (auto &c : coord) c=float(input->readLong(4))/65536;
        listPts[p].push_back(MWAWVec2f(coord[1],coord[0]));
        f << listPts[p].back();
        if (!f2.str().empty()) f << "[" << f2.str() << "]";
        if (j+1==nbPts) f << ",";
        else f << ":";
...
bool MacDraft5Parser::readText(MacDraft5ParserInternal::Shape &shape)
{
  ...
  if (ok) {
    ...
    tSz=static_cast<int>(input->readULong(2));
  ...
  if (ok) {
    ...
    nData=static_cast<int>(input->readULong(2));
  }
  ...
  for (int i=0; i<nData; ++i) {
    ...
    f << "pos=" << cPos << ",";
    auto val=static_cast<int>(input->readLong(2)); // c-2f
    if (val) f << "f0=" << val << ",";
...
bool MacDraft5Parser::readModifier(MacDraft5ParserInternal::Shape &shape)
{
  ...
  for (int i=0; i<N; ++i) {
    ...
    auto value=long(input->readULong(4));
    f << "mod" << i << "=[";
    switch (type) {
    case 0:
      if (value==0)
...
bool MacDraft5Parser::readViews(MWAWEntry const &entry, bool inRsrc)
{
  ...
  val=static_cast<int>(input->readLong(2)); // always 0 ?
  if (val) f << "f3=" << val << ",";
  val=static_cast<int>(input->readLong(2)); // always 38
  if (val!=38) f << "#fSz=" << val << ",";
  long dataSz=input->readLong(4);
  if (dataSz!=38*N) {
  ...
  for (long i=0; i<N; ++i) {
    ...
    input->seek(pos+32, librevenge::RVNG_SEEK_SET);
    int coord[2];
    for (auto &c : coord) c=static_cast<int>(input->readULong(2));
    f << MWAWVec2i(coord[0],coord[1]) << ",";
...
bool MacDraft5Parser::readLayoutDefinitions(MWAWEntry const &entry, bool inRsrc)
{
  ...
  ascFile.addPos(entry.begin()-(inRsrc ? 4 : 16));
  ascFile.addNote(f.str().c_str());
  /* Normally, we can find one layout definition in the data fork and one in the resource.
     So, we must clear the layout list to avoid dupplicating each layout
   */
  m_state->m_layoutList.clear();
  for (long i=0; i<N; ++i) {
    long pos=input->tell();
    f.str("");
    std::shared_ptr<MacDraft5ParserInternal::Layout> layout(new MacDraft5ParserInternal::Layout(int(m_state->m_layoutList.size())));
    for (int c=0; c<16; ++c) {
      auto ch=char(input->readULong(1));
      if (ch==0) break;
      int unicode= getParserState()->m_fontConverter->unicode(3, static_cast<unsigned char>(ch));
      if (unicode==-1)
        ...
        libmwaw::appendUnicode(static_cast<uint32_t>(unicode), layout->m_name);
    }
    ...
    layout->m_N=static_cast<int>(input->readULong(4));
...
bool MacDraft5Parser::send(MacDraft5ParserInternal::Image const &image)
{
  ...
  for (auto layout : m_state->m_layoutList) {
    if (!layout) continue;
    std::shared_ptr<MacDraft5ParserInternal::Shape> shape=layout->findShape(image.m_id, false);
    if (!shape) continue;
    ok=true;
...
bool MacDraft5Parser::send(MacDraft5ParserInternal::Layout const &layout)
{
  ...
  if (!layout.m_name.empty())
    openLayer=listener->openLayer(layout.m_name);
  for (auto id : layout.m_rootList) {
    auto child=layout.m_shapeList[id];
                           ...
                           MacDraft5ParserInternal::Layout const &layout)
{
  ...
  case MacDraft5ParserInternal::Shape::Basic:
    ...
    for (size_t i=0; i<shape.m_otherShapeList.size(); ++i) {
      ...
      pos.m_anchorTo = MWAWPosition::Page;
      listener->insertShape(pos, shape.m_otherShapeList[i], shape.m_otherStyleList[i]);
    }
    break;
  case MacDraft5ParserInternal::Shape::Text: {
    std::shared_ptr<MWAWSubDocument> doc(new MacDraft5ParserInternal::SubDocument(*this, getInput(), layout.m_id, shape.m_id));
    listener->insertTextBox(pos, doc, style);
    break;
  }
  case MacDraft5ParserInternal::Shape::QuotationSurface: {
    pos=MWAWPosition(shape.m_origin+m_state->m_origin, MWAWVec2f(-1,-1), librevenge::RVNG_POINT);
    pos.m_anchorTo = MWAWPosition::Page;
    std::shared_ptr<MWAWSubDocument> doc(new MacDraft5ParserInternal::SubDocument(*this, getInput(), layout.m_id, shape.m_id));
    listener->insertTextBox(pos, doc, style);
    break;
  ...
  for (auto id : shape.m_childList) {
    auto child=layout.findShape(id);
    if (!child) continue;
    if (child->m_isSent) {
...
bool MacDraft5Parser::sendBitmap(MacDraft5ParserInternal::Shape const &bitmap, MWAWPosition const &position)
{
  ...
  if (!m_styleManager->getBitmap(bitmap.m_bitmapIdList[0], picture)) return false;
...
bool MacDraft5Parser::sendText(int layoutId, long zId)
{
  ...
  auto const &layout=*m_state->m_layoutList[size_t(layoutId)];
  auto shape=layout.findShape(zId);
  if (!shape)
    return false;
  shape->m_isSent = true;
