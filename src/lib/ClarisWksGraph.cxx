...
struct CurvePoint {
  explicit CurvePoint(MWAWVec2f point=MWAWVec2f())
    : m_pos(point)
    , m_type(1)
  {
    for (auto &ctrl : m_controlPoints) ctrl = point;
  }
  //! operator<<
...
struct Style final : public MWAWGraphicStyle {
  //! constructor
  Style()
    : MWAWGraphicStyle()
    , m_wrapping(0)
    , m_surfacePatternType(0)
  {
  }
...
struct ZoneShape final : public Zone {
  //! constructor
...
struct ZonePict final : public Zone {
  //! constructor
...
struct Bitmap final : public ClarisWksStruct::DSET {
  //! constructor
  explicit Bitmap(ClarisWksStruct::DSET const &dset = ClarisWksStruct::DSET()) :
...
struct ZoneZone final : public Zone {
  //! constructor
...
struct Chart final : public Zone {
  //! construtor
  explicit Chart(Zone const &z) : Zone(z)
  {
  }
  //! destructor
  ~Chart() final;
...
struct ZoneUnknown final : public Zone {
  //! construtor
  explicit ZoneUnknown(Zone const &z)
    : Zone(z)
    , m_type(T_Unknown)
    , m_typeId(-1)
  {
...
struct Group final : public ClarisWksStruct::DSET {
  //! constructor
  explicit Group(ClarisWksStruct::DSET const &dset = ClarisWksStruct::DSET())
  ...
  void removeChild(int cId, bool normalChild) final
  {
    ...
    for (auto it=m_zones.begin(); it!=m_zones.end(); ++it) {
      ...
      if (!child || child->getType() != ClarisWksGraphInternal::Zone::T_Zone)
        continue;
      auto &childZone = static_cast<ClarisWksGraphInternal::ZoneZone &>(*child);
...
struct State {
  ...
  State()
    : m_numPages(0)
    , m_pageDimensions(0,0)
    , m_masterId(-1)
    , m_groupMap()
    , m_bitmapMap()
    , m_databaseMap()
    , m_frameId(0)
    , m_positionsComputed(false)
    , m_ordering(0) { }
...
ClarisWksGraph::ClarisWksGraph(ClarisWksDocument &document)
  ...
  , m_state(new ClarisWksGraphInternal::State)
...
void ClarisWksGraph::computePositions() const
{
  ...
  for (auto iter : m_state->m_groupMap) {
...
////////////////////////////////////////////////////////////
std::shared_ptr<ClarisWksStruct::DSET> ClarisWksGraph::readGroupZone
(ClarisWksStruct::DSET const &zone, MWAWEntry const &entry, bool &complete)
{
  ...
  std::shared_ptr<ClarisWksGraphInternal::Group> group(new ClarisWksGraphInternal::Group(zone));

  f << "Entries(GroupDef):" << *group << ",";
  auto val = static_cast<int>(input->readLong(2)); // a small int between 0 and 3
  switch (val) {
  case 0:
    break; // normal
  case 3:
  ...
  val = static_cast<int>(input->readLong(2)); // a small number between 0 and 1e8
  if (val) f << "f1=" << val << ",";
...
// read a bitmap zone
////////////////////////////////////////////////////////////
std::shared_ptr<ClarisWksStruct::DSET> ClarisWksGraph::readBitmapZone
(ClarisWksStruct::DSET const &zone, MWAWEntry const &entry, bool &complete)
{
  complete = false;
  ...
  libmwaw::DebugFile &ascFile = m_parserState->m_asciiFile;
  libmwaw::DebugStream f;
  std::shared_ptr<ClarisWksGraphInternal::Bitmap> bitmap(new ClarisWksGraphInternal::Bitmap(zone));
...
{
  if (!m_parserState) {
    MWAW_DEBUG_MSG(("ClarisWksGraph::readDatabasePictZone: can not find the parser state\n"));
    return false;
  }
  MWAWInputStreamPtr input = m_parserState->m_input;
  long pos = input->tell();
  auto endPos = long(input->readULong(4));
  if (endPos<pos || !input->checkPosition(endPos)) {
    input->seek(pos, librevenge::RVNG_SEEK_SET);
    MWAW_DEBUG_MSG(("ClarisWksGraph::readDatabasePictZone: unexpected size\n"));
    return false;
  }
  libmwaw::DebugFile &ascFile=m_parserState->m_asciiFile;
  libmwaw::DebugStream f;
  ...
  pos=input->tell();
  long firstPos=pos+0x200-(pos%0x200);
  if (firstPos!=pos) {
    ascFile.skipZone(pos, firstPos-1);
  ...
  for (int i=0; i<2 && ok; ++i) {
    pos=input->tell();
    // unsure, find 0000000c0000ffff000000080000|0001, so maybe simply two zones of size 16
    ok=ClarisWksStruct::readStructZone(*m_parserState, "DataPict", false);
    if (!ok)
      input->seek(pos, librevenge::RVNG_SEEK_SET);
  }
  struct PictEntry {
    PictEntry()
    {
      for (auto &v : m_pos) v=-1;
    }
    long m_pos[3];
  };
  std::vector<PictEntry> pictEntryList;
  if (ok) {
    f.str("");
    f << "DataPict(I):";
    pos = input->tell();
    ...
    f << "N=" << N << ",";
    if (pos+16+28*N+4>endPos) {
      MWAW_DEBUG_MSG(("ClarisWksGraph::readDatabasePictZone: the number of zones seems bad\n"));
      f << "###N,";
      N=0;
      ok=false;
    }
    ...
    for (int i=0; i<N; ++i) {
      ...
      f << "DataPict(I" << i << "):";
      PictEntry data;
      data.m_pos[0]=long(input->readULong(4));
      f << "dSz=" << data.m_pos[0] << ",";
      for (int j=0; j<8; ++j) {
        auto val=int(input->readLong(2));
        int const expected[] = {0,0x14,1,-1,0,8,0,1}; // some struct?
        if (val!=expected[j])
          f << "f" << i << "=" << val << ",";
      }
      for (int j=0; j<2; ++j) data.m_pos[j+1]=long(input->readULong(4));
      f << "zone=" << data.m_pos[1] << "x" << data.m_pos[2] << ",";
      if (data.m_pos[1]<1 || data.m_pos[2]<data.m_pos[1] || (endPos-firstPos)/0x200 < data.m_pos[2]+1
          || data.m_pos[0]>0x200*(data.m_pos[2]+1-data.m_pos[1])) {
        MWAW_DEBUG_MSG(("ClarisWksGraph::readDatabasePictZone: the %d zone seems bad\n", i));
        f << "###";
        pictEntryList.push_back(PictEntry());
      }
      else
        pictEntryList.push_back(data);
    ...
    if (ok) {
      ...
      ascFile.addNote(f.str().c_str());
      pos=input->tell();
      long newPos=pos+0x200-(pos%0x200);
      if (newPos!=pos) {
        ascFile.skipZone(pos, newPos-1);
        input->seek(newPos, librevenge::RVNG_SEEK_SET);
      }
    }
  }
  for (size_t i=0; i<pictEntryList.size(); ++i) {
    auto const &entry = pictEntryList[i];
    if (entry.m_pos[0]<=0) continue;
    long begDataPos=firstPos+entry.m_pos[1]*0x200;
    input->seek(begDataPos, librevenge::RVNG_SEEK_SET);
    long endDataPos=begDataPos+entry.m_pos[0];
    unsigned long type=input->readULong(4);
    std::shared_ptr<ClarisWksGraphInternal::ZonePict> pict;
    input->seek(-4, librevenge::RVNG_SEEK_CUR);
    if (type==0x5154494d) {
      pict.reset(new ClarisWksGraphInternal::ZonePict(ClarisWksGraphInternal::Zone(), ClarisWksGraphInternal::Zone::T_QTim));
      ok=readQTimeData(pict);
    }
    else {
      pict.reset(new ClarisWksGraphInternal::ZonePict(ClarisWksGraphInternal::Zone(), ClarisWksGraphInternal::Zone::T_Pict));
      ok=readPictData(pict);
    }
    if (ok && pict)
      m_state->m_databaseMap[int(i)] = pict;
    if (ok) {
      pos=input->tell();
      auto pictSz=long(input->readULong(4));
      if (pictSz<0 || pictSz>endDataPos-pos-4)
        input->seek(pos, librevenge::RVNG_SEEK_SET);
      else if (!pictSz) {
        ascFile.addPos(pos);
        ascFile.addNote("Nop");
        ...
        ascFile.addPos(pos);
        ascFile.addNote("PictData[unkn]");
        input->seek(pos+4+pictSz, librevenge::RVNG_SEEK_SET);
      }
    }
    if (input->tell()!=endDataPos) {
      ascFile.addPos(input->tell());
      ascFile.addNote("DataPict[data]:###extra");
      MWAW_DEBUG_MSG(("ClarisWksGraph::readDatabasePictZone: find extra picture data\n"));
      input->seek(endDataPos, librevenge::RVNG_SEEK_SET);
    }
    long endZonePos = firstPos+(entry.m_pos[2]+1)*0x200;
    if (ok && input->tell()!=endZonePos) {
      ascFile.skipZone(input->tell(), endZonePos-1);
      input->seek(endZonePos, librevenge::RVNG_SEEK_SET);
    }
  }
  ...
  pos=input->tell();
  if (pos<endPos) {
    MWAW_DEBUG_MSG(("ClarisWksGraph::readDatabasePictZone: find extra data\n"));
    ascFile.addPos(pos);
    ascFile.addNote("DataPict:##extra");
  }
...
{
  // master page does not seems to exist in v1-v2 documents
  if (m_parserState->m_kind != MWAWDocument::MWAW_K_DRAW || version() <= 3)
    return;
  auto main=m_document.getZone(1);
  if (m_state->m_groupMap.find(1)== m_state->m_groupMap.end())
    return;
  auto mainGroup=m_state->m_groupMap.find(1)->second;
  if (!main || !mainGroup)
    return;
  // search for the master block
  int masterBlockId=-1;
  MWAWVec2f pageDim(0,0);
  for (auto zone : mainGroup->m_zones) {
    if (!zone || zone->getType()!=ClarisWksGraphInternal::Zone::T_Zone ||
        (zone->m_zoneType&0xF)!=0xa)
      continue;
    if (masterBlockId!=-1) {
      MWAW_DEBUG_MSG(("ClarisWksGraph::findMasterPage: a master block is already found\n"));
      return;
    }
    MWAWBox2f box=zone->getBdBox();
    if (box[0][0]<=0 && box[0][0]>=0 && box[0][1]<=0 && box[0][1]>=0)
      pageDim=box.size();
    else {
      MWAW_DEBUG_MSG(("ClarisWksGraph::findMasterPage: the page dimension seems odd\n"));
    }
    masterBlockId=static_cast<ClarisWksGraphInternal::ZoneZone const &>(*zone).m_id;
  }
  if (masterBlockId==-1) return;
  m_state->m_pageDimensions=pageDim;
  auto master=m_document.getZone(masterBlockId);
  if (!master || master->m_fathersList.size()!=1 || *master->m_fathersList.begin()!=1  ||
      m_state->m_groupMap.find(masterBlockId)==m_state->m_groupMap.end() || !m_state->m_groupMap.find(masterBlockId)->second) {
    MWAW_DEBUG_MSG(("ClarisWksGraph::findMasterPage: can not find the master zone\n"));
    return;
  }
  m_state->m_masterId=masterBlockId;
...
{
  std::shared_ptr<ClarisWksGraphInternal::Zone> res;
  if (entry.length() < 32) {
  ...
  auto &style = zone.m_style;

  auto typeId = static_cast<int>(input->readULong(1));
  auto type = ClarisWksGraphInternal::Zone::T_Unknown;
  ...
  if (vers > 1)
    ...
    f << "g0=" << (val>>2) << ",";
  zone.m_zoneType = static_cast<int>(input->readULong(1));
  ...
  float dim[4];
  for (auto &d : dim) {
    d = float(input->readLong(4))/256.f;
    if (d < -100) f << "##dim?,";
  }
  ...
  if (val) f << "f1=" << val << ",";
  for (int j = 0; j < 2; j++) {
    auto col = static_cast<int>(input->readULong(1));
    MWAWColor color;
  ...
  for (int j = 0; j < 2; j++) {
    if (vers > 1) {
      val = static_cast<int>(input->readULong(1));  // probably also related to surface
      if (val) f << "pat" << j << "[high]=" << val << ",";
    }
    auto pat = static_cast<int>(input->readULong(1));
    if (j==1 && style.m_surfacePatternType) {
      ...
      else { // find one time 2 associtated with a picture
        f << "###surf[patternType]=" << style.m_surfacePatternType << ",";
        MWAW_DEBUG_MSG(("ClarisWksGraph::readGroupDef: unknown surface pattern type!!!\n"));
      }
      continue;
    }
  ...
  zone.m_ordering = static_cast<int>(input->readLong(2));

  switch (type) {
  case ClarisWksGraphInternal::Zone::T_Zone: {
    int nFlags = 0;
    auto z = std::make_shared<ClarisWksGraphInternal::ZoneZone>(zone, type);
    res = z;
    z->m_flags[nFlags++] = static_cast<int>(input->readLong(2));
    z->m_id = static_cast<int>(input->readULong(2));

    auto numRemains = int(entry.end()-long(input->tell()));
    numRemains/=2;
    // v1-2:3 v4-v5:6 v6:8
  ...
  case ClarisWksGraphInternal::Zone::T_Poly: {
    auto z = std::make_shared<ClarisWksGraphInternal::ZoneShape>(zone, type);
    res = z;
    readShape(entry, *z);
    break;
  }
...
#endif
  {
    auto z = std::make_shared<ClarisWksGraphInternal::ZoneUnknown>(zone);
    res = z;
    z->m_type = type;
    z->m_typeId = typeId;
    break;
  ...
  for (auto z : group.m_zones) {
    ...
    if (numZoneExpected) {
      ...
      case ClarisWksGraphInternal::Zone::T_Zone2: {
        auto *child=dynamic_cast<ClarisWksGraphInternal::ZoneZone *>(z.get());
        if (!child) {
        ...
        val=static_cast<int>(input->readULong(2)); // 6e
        if (val) f << "f2=" << val << ",";
        f << "ids=[";
        for (int j=0; j<2; ++j)
          f << input->readULong(4) << ",";;
        ...
        if (val) f << "f3=" << val << ",";
        child->m_id=static_cast<int>(input->readULong(2));
        f << "child=" << child->m_id << ",";
        ascFile.addDelimiter(input->tell(),'|');
        ascFile.addPos(pos);
        ascFile.addNote(f.str().c_str());
...
bool ClarisWksGraph::readShape(MWAWEntry const &entry, ClarisWksGraphInternal::ZoneShape &zone)
{
  MWAWInputStreamPtr &input= m_parserState->m_input;
  long actPos = input->tell();
  auto remainBytes = int(entry.end()-actPos);
  if (remainBytes < 0)
    return false;

  MWAWVec2f pictSz=zone.getBdBox().size();
  MWAWBox2f box(MWAWVec2f(0,0), pictSz);
  ...
  case ClarisWksGraphInternal::Zone::T_Arc: {
    ...
    int fileAngle[2];
    for (int &fAngle : fileAngle) fAngle = static_cast<int>(input->readLong(2));
  ...
  auto numRemain = int(nextToRead-input->tell());
  numRemain /= 2;
  ...
  if (vers>=4 && input->tell() <= nextToRead) {
    ...
    zone.m_rotate=static_cast<int>(input->readLong(2));
    if (zone.m_rotate) {
...
bool ClarisWksGraph::readGroupHeader(ClarisWksGraphInternal::Group &group)
{
  ...
  if (!header.readHeader(input,false)) {
    MWAW_DEBUG_MSG(("ClarisWksGraph::readGroupHeader: can not read the header\n"));
    return false;
  }
  libmwaw::DebugFile &ascFile = m_parserState->m_asciiFile;
  libmwaw::DebugStream f;
  ...
  for (long i = 0; i < header.m_numData; i++) {
    pos = input->tell();
    if (readGroupUnknown(group, int(header.m_dataSize), int(i)))
      continue;
    ascFile.addPos(pos);
    ascFile.addNote("GroupDef(Head-###)");
  ...
  int const numHeaders= header.m_numData==0 ? 1 : int(header.m_numData);
...
bool ClarisWksGraph::readGroupUnknown(ClarisWksGraphInternal::Group &group, int zoneSz, int id)
{
  ...
  input->seek(pos, librevenge::RVNG_SEEK_SET);
  auto type = static_cast<int>(input->readLong(2)); // find -1, 0, 3
  if (type) f << "f0=" << type << ",";
  for (int i = 0; i < 6; i++) {
  ...
  for (int i = 0; i < 2; i++)
    values32.push_back(static_cast<int32_t>(input->readLong(4)));
  ...
  MWAWVec2i dim(static_cast<int>(values32[0]),static_cast<int>(values32[1]));
  if (id < 0)
    group.m_pageDimension=MWAWVec2f(dim);
...
bool ClarisWksGraph::readPolygonData(std::shared_ptr<ClarisWksGraphInternal::Zone> zone)
{
  if (!zone || zone->getType() != ClarisWksGraphInternal::Zone::T_Shape)
    return false;
  auto *bZone = static_cast<ClarisWksGraphInternal::ZoneShape *>(zone.get());
  ...
  ClarisWksStruct::Struct header;
  if (!header.readHeader(input,false)) {
    MWAW_DEBUG_MSG(("ClarisWksGraph::readPolygonData: can not read the header\n"));
    return false;
  }
  libmwaw::DebugFile &ascFile = m_parserState->m_asciiFile;
  libmwaw::DebugStream f;
  ...
  for (long i = 0; i < header.m_numData; i++) {
    ...
    f << "PolygonData-" << i << ":";
    float position[2];
    for (float &posi : position) posi = float(input->readLong(4))/256.f;
    ClarisWksGraphInternal::CurvePoint point(MWAWVec2f(position[1], position[0]));
    if (header.m_dataSize >= 26) {
      for (auto &controlPoint : point.m_controlPoints) {
        float ctrlPos[2];
        for (auto &ctrl : ctrlPos) ctrl = float(input->readLong(4))/256.f;
        controlPoint = MWAWVec2f(ctrlPos[1], ctrlPos[0]);
      }
      auto fl = static_cast<int>(input->readULong(2));
      point.m_type = (fl>>14);
      f << point << ",";
      if (fl&0x3FFF)
  ...
  if (!isSpline) {
    ...
    for (auto const &vertex : vertices)
      shape.m_vertices.push_back(vertex.m_pos);
    return true;
  }
...
bool ClarisWksGraph::readChartData(std::shared_ptr<ClarisWksGraphInternal::Zone> zone)
{
  ...
  long pos = input->tell();
  ClarisWksStruct::Struct header;
  if (!header.readHeader(input,true) || header.m_size==0 || header.m_dataSize<16) {
    MWAW_DEBUG_MSG(("ClarisWksGraph::readChartData: can not read the header\n"));
    return false;
  }
  libmwaw::DebugFile &ascFile = m_parserState->m_asciiFile;
  libmwaw::DebugStream f;
  long endPos=pos+4+header.m_size;
  f << "Entries(ChartData):" << header;
  if (header.m_headerSize) {
    ascFile.addDelimiter(input->tell(),'|');
    input->seek(header.m_headerSize, librevenge::RVNG_SEEK_CUR);
  }
...
bool ClarisWksGraph::readPICT(ClarisWksGraphInternal::ZonePict &zone)
{
  ...
  MWAWBox2f box;
  auto res = MWAWPictData::check(input, static_cast<int>(sz), box);
...
bool ClarisWksGraph::readPS(ClarisWksGraphInternal::ZonePict &zone)
{
  ...
  long pos = input->tell();
  auto sz = long(input->readULong(4));
  auto header = long(input->readULong(4));
...
bool ClarisWksGraph::readOLE(ClarisWksGraphInternal::ZonePict &zone)
{
  ...
  f << "Entries(OLE):";
  int dim[4];
  for (auto &d : dim) d = static_cast<int>(static_cast<int32_t>(MWAWInputStream::readULong(input->input().get(), 4, 0, true)));
...
bool ClarisWksGraph::readBitmapColorMap(std::vector<MWAWColor> &cMap)
{
  ...
  auto maxColor = static_cast<int>(input->readLong(4));
  if (sz != 8+8*(long(maxColor)+1)) {
...
bool ClarisWksGraph::readBitmapData(ClarisWksGraphInternal::Bitmap &zone)
{
  ...
  auto numBytesPerPixel = int(sz/numPixels);
...
void ClarisWksGraph::updateGroup(ClarisWksGraphInternal::Group &group) const
{
  ...
  for (size_t g = 0; g < group.m_zones.size(); g++) {
    auto child = group.m_zones[g];
    if (!child) continue;
    if (child->getType() == ClarisWksGraphInternal::Zone::T_Zone) {
      auto const &childZone = static_cast<ClarisWksGraphInternal::ZoneZone &>(*child);
      int zId = childZone.m_id;
      std::shared_ptr<ClarisWksStruct::DSET> zone=m_document.getZone(zId);
      if (!zone)
        continue;
      ...
      auto &sIdCIdMap=idSIdCIdMap.find(zId)->second;
  ...
  for (auto it : idSIdCIdMap) {
    auto const &sIdCIdMap=it.second;
    if (sIdCIdMap.size()<=1) continue;
    int frameId=m_state->m_frameId++;
    int frameSubId=0;
    for (auto cIt=sIdCIdMap.begin(); cIt!=sIdCIdMap.end();) {
      auto *child = static_cast<ClarisWksGraphInternal::ZoneZone *>(group.m_zones[cIt++->second].get());
...
////////////////////////////////////////////////////////////
bool ClarisWksGraph::sendGroupChild(std::vector<std::shared_ptr<ClarisWksGraphInternal::Zone> > const &lChild, MWAWListenerPtr listener, MWAWVec2f const &leftTop)
{
  if (!listener) {
    MWAW_DEBUG_MSG(("ClarisWksGraph::sendGroupChild: can not find the listener!!!\n"));
    return false;
  }
  for (auto child : lChild) {
    if (!child) continue;
    ...
    if (type==ClarisWksGraphInternal::Zone::T_Zone) {
      ...
      auto dset=m_document.getZone(zId);
      if (!dset) continue;
      if (dset->m_fileType==4) {
        sendBitmap(zId, listener, pos);
        continue;
      }
      std::shared_ptr<MWAWSubDocument> doc(new ClarisWksGraphInternal::SubDocument(*this, m_parserState->m_input, zId));
      if (dset->m_fileType==1) // text box
        listener->insertTextBox(pos, doc, child->m_style);
      else if (dset->m_fileType==2 || dset->m_fileType==6) { // spreadsheet/table
        bool createFrame=listener->openFrame(pos, child->m_style);
        m_document.sendZone(zId, listener, pos);
        if (createFrame)
          listener->closeFrame();
        return true;
      }
      else if (dset->m_fileType==0) {
        if (listener->getType()==MWAWListener::Graphic)
          static_cast<MWAWGraphicListener *>(listener.get())->insertGroup(box, doc);
        else if (listener->getType()==MWAWListener::Presentation)
          static_cast<MWAWPresentationListener *>(listener.get())->insertGroup(box, doc);
        else {
          MWAW_DEBUG_MSG(("ClarisWksGraph::sendGroupChild: do not known how to send a group data!!!\n"));
        }
      }
      else {
        MWAW_DEBUG_MSG(("ClarisWksGraph::sendGroupChild: find unexpected zone type!!!\n"));
      }
      continue;
    }
    else if (type==ClarisWksGraphInternal::Zone::T_Shape) {
      auto const &shape=static_cast<ClarisWksGraphInternal::ZoneShape const &>(*child);
      ...
      MWAW_DEBUG_MSG(("ClarisWksGraph::sendGroupChild: find unexpected type!!!\n"));
    }
  }
  return true;
}
...
bool ClarisWksGraph::sendPageChild(ClarisWksGraphInternal::Group &group)
{
  ...
  for (auto child : group.m_zonesToSend) {
    if (!child || child->m_page<=0) continue;
    if (child->getType() == ClarisWksGraphInternal::Zone::T_Zone) {
      auto dset=m_document.getZone(child->getZoneId());
...
bool ClarisWksGraph::sendGroup(ClarisWksGraphInternal::Group &group, MWAWPosition const &position)
{
  ...
  if (!mainGroup && canSendAsGraphic(group)) {
    ...
    sendGroupChild(group.m_zonesToSend, graphicListener, leftTop);
  ...
  std::vector<std::shared_ptr<ClarisWksGraphInternal::Zone> > listJobs[2];
  for (auto child : group.m_zonesToSend) {
    if (!child) continue;
    ...
    if (child->getType() == ClarisWksGraphInternal::Zone::T_Zone) {
      std::shared_ptr<ClarisWksStruct::DSET> dset=m_document.getZone(child->getZoneId());
      if (dset && dset->m_position==ClarisWksStruct::DSET::P_Main) {
        listJobs[1].push_back(child);
        continue;
      }
  ...
  for (int st = 0; st < 2; st++) {
    ...
    for (size_t g = 0; g < numJobs; g++) {
      MWAWBox2f box;
      std::vector<std::shared_ptr<ClarisWksGraphInternal::Zone> > groupList;
      int page = 0;
      size_t lastOk=g;

      if (st==0 && !mainGroup) {
        for (size_t h = g; h < numJobs; ++h) {
          auto child = listJobs[st][h];
          if (!child) continue;
      ...
      MWAWGraphicListenerPtr graphicListener(new MWAWGraphicListener(*m_parserState, box, &graphicEncoder));
      graphicListener->startDocument();
      sendGroupChild(groupList, graphicListener, leftTop);
      graphicListener->endDocument();
      MWAWEmbeddedObject picture;
      if (graphicEncoder.getBinaryResult(picture)) {
        MWAWGraphicStyle style(MWAWGraphicStyle::emptyStyle());
...
bool ClarisWksGraph::sendGroupChild(std::shared_ptr<ClarisWksGraphInternal::Zone> child, MWAWPosition pos)
{
  ...
  auto const &cStyle = child->m_style;
  ...
  auto cPos=dset ? dset->m_position : ClarisWksStruct::DSET::P_Unknown;
  bool createFrame=
  ...
  if (dset && dset->m_fileType==1) { // checkme: use style for textbox
    ...
    if (cStyle.hasLine()) {
      ...
      auto extend = float(cStyle.m_lineWidth*0.85f);
      pos.setOrigin(pos.origin()-MWAWVec2f(extend,extend));
      pos.setSize(pos.size()+2.0f*MWAWVec2f(extend,extend));
    }
  }
  else
    style.m_backgroundOpacity=0;
  bool isPresentation=listener->getType()==MWAWListener::Presentation;
  ...
  if (isPresentation && createFrame) {
    std::shared_ptr<MWAWSubDocument> doc(new ClarisWksGraphInternal::SubDocument(*this, m_parserState->m_input, zId));
    static_cast<MWAWPresentationListener *>(listener.get())->insertGroup(childZone.m_box, doc);
    return true;
  }
...
bool ClarisWksGraph::sendShape(ClarisWksGraphInternal::ZoneShape &pict, MWAWPosition pos)
{
  ...
  listener->insertShape(pos, pict.m_shape, pStyle);
  return true;
}
...
bool ClarisWksGraph::canSendBitmapAsGraphic(int number) const
{
  if (m_parserState->m_kind == MWAWDocument::MWAW_K_PRESENTATION)
    return false;
  auto iter = m_state->m_bitmapMap.find(number);
...
bool ClarisWksGraph::sendDatabasePictZone(int pictId, MWAWListenerPtr listener, MWAWPosition const &pos)
{
  auto iter = m_state->m_databaseMap.find(pictId);
  if (iter == m_state->m_databaseMap.end() || !iter->second) {
    MWAW_DEBUG_MSG(("ClarisWksGraph::sendDatabasePictZone: can not find zone %d\n", pictId));
    return false;
  }
  if (!listener)
    listener=m_parserState->getMainListener();
  if (!listener) {
    MWAW_DEBUG_MSG(("ClarisWksGraph::sendDatabasePictZone: can not find a listener\n"));
    return false;
  }
  return sendPicture(*iter->second, pos);
}
...
bool ClarisWksGraph::sendBitmap(ClarisWksGraphInternal::Bitmap &bitmap, MWAWListener &listener, MWAWPosition pos)
{
  ...
  auto numColors = int(bitmap.m_colorMap.size());
  std::shared_ptr<MWAWPictBitmap> bmap;

  MWAWPictBitmapIndexed *bmapIndexed = nullptr;
  MWAWPictBitmapColor *bmapColor = nullptr;
  bool indexed = false;
  if (numColors > 2) {
  ...
  for (int r = 0; r < bitmap.m_bitmapSize[1]; r++) {
    ...
    for (int c = 0; c < bitmap.m_bitmapSize[0]; c++) {
      ...
      if (indexed) {
        bmapIndexed->set(c,r,static_cast<int>(val));
        continue;
      }
      switch (bytesPerPixel) {
      case 1:
        bmapColor->set(c,r, MWAWColor(static_cast<unsigned char>(val),static_cast<unsigned char>(val),static_cast<unsigned char>(val)));
        break;
      case 2: // rgb compressed ?
        bmapColor->set(c,r, MWAWColor(static_cast<unsigned char>(((val>>10)&0x1F) << 3),static_cast<unsigned char>(((val>>5)&0x1F) << 3),static_cast<unsigned char>(((val>>0)&0x1F) << 3)));
        break;
      case 4:
        bmapColor->set(c,r, MWAWColor(static_cast<uint32_t>(val)|0xFF000000)); // checkme
        break;
      default: {
  ...
  MWAWEmbeddedObject picture;
  if (!bmap->getBinary(picture)) return false;
  if (pos.size()[0] <= 0 || pos.size()[1] <= 0) {
  ...
  for (auto entry : pict.m_entries) {
    ...
    default:
      ...
      if (!send && listener) {
        ...
        listener->insertPicture(pos, MWAWEmbeddedObject(data, "image/pict"));
      }
      send = true;
      break;
    }
...
{
  auto iter = m_state->m_groupMap.find(number);
...
bool ClarisWksGraph::canSendAsGraphic(ClarisWksGraphInternal::Group &group) const
{
  ...
  for (size_t g = 0; g < numZones; g++) {
    auto child = group.m_zonesToSend[g];
    if (!child) continue;
...
bool ClarisWksGraph::sendMaster(int pg)
{
  if (m_parserState->m_kind != MWAWDocument::MWAW_K_DRAW || m_state->m_masterId<=0)
    return true;
  auto iter = m_state->m_groupMap.find(m_state->m_masterId);
  if (iter == m_state->m_groupMap.end() || !iter->second) {
  ...
  for (auto &zone : group->m_zones) {
    std::shared_ptr<ClarisWksGraphInternal::Zone> child = zone;
    if (!child) continue;
    if (child->getType() == ClarisWksGraphInternal::Zone::T_Zone) {
      auto dset=m_document.getZone(child->getZoneId());
      if (dset && dset->isHeaderFooter()) {
        zone.reset();
        MWAW_DEBUG_MSG(("ClarisWksGraph::sendMaster: remove some header footer from master\n"));
        continue;
      }
    }
    child->m_page = pg+1;
...
{
  auto iter = m_state->m_groupMap.find(groupId);
  if (iter == m_state->m_groupMap.end() || !iter->second) {
    MWAW_DEBUG_MSG(("ClarisWksGraph::sendPageGraphics: can not find the group %d\n", groupId));
    return false;
  }
  auto group = iter->second;
  if (group->m_position==ClarisWksStruct::DSET::P_GraphicMaster) return true;
  group->m_parsed=true;
  return sendPageChild(*group);
}
