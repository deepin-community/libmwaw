...
bool GreatWksDocument::readWPSN(MWAWEntry const &entry)
{
  ...
  for (int i=0; i < N; ++i) {
    ...
    f << "Windows-" << i << ":";
    int width[2];
    for (auto &w : width) w=static_cast<int>(input->readLong(2));
    f << "w=" << width[1] << "x" << width[0] << ",";
    int LT[2];
    for (auto &orig : LT) orig=static_cast<int>(input->readLong(2));
    f << "LT=" << LT[1] << "x" << LT[0] << ",";
...
bool GreatWksDocument::readGrDS(MWAWEntry const &entry)
{
  ...
  for (int i=0; i < N; ++i) {
    ...
    f.str("");
    f << "GrDS-" << i << ":";
    auto val=static_cast<int>(input->readLong(2)); // 1,2,3
    f << "unkn=" << val << ",";
    for (int st=0; st < 2; ++st) {
      unsigned char col[3];
...
bool GreatWksDocument::readString(long endPos, std::string &res)
{
  ...
  if (pos+1+fSz>endPos || !input->checkPosition(endPos)) {
...
bool GreatWksDocument::readFormula(MWAWVec2i const &cPos, long endPos, std::vector<MWAWCellContent::FormulaInstruction> &formula, std::string &error)
{
  ...
  while (!input->isEnd()) {
    ...
    if (pos >= endPos)
      break;
    int arity=0, val, type=static_cast<int>(input->readULong(1));
