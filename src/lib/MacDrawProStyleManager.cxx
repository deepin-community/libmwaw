...
struct Gradient {
  ...
  {
    return m_type != MWAWGraphicStyle::Gradient::G_None && static_cast<int>(m_stopList.size()) >= 2;
  }
  //! a print operator
...
std::ostream &operator<<(std::ostream &o, Gradient const &grad)
{
  ...
  switch (grad.m_type) {
  case MWAWGraphicStyle::Gradient::G_Axial:
    o << "axial,";
    break;
  case MWAWGraphicStyle::Gradient::G_Linear:
    o << "linear,";
    break;
  case MWAWGraphicStyle::Gradient::G_Radial:
    o << "radial,";
    break;
  case MWAWGraphicStyle::Gradient::G_Rectangular:
    o << "rectangular,";
    break;
  case MWAWGraphicStyle::Gradient::G_Square:
    o << "square,";
    break;
  case MWAWGraphicStyle::Gradient::G_Ellipsoid:
    o << "ellipsoid,";
    break;
  case MWAWGraphicStyle::Gradient::G_None:
...
struct State {
  ...
  State()
    ...
    , m_numColors(8)
    , m_numBWPatterns(0)
    , m_numColorPatterns(0)
    , m_numPatternsInTool(0)
    , m_colorList()
    , m_displayColorList()
    , m_dashList()
    , m_fontList()
    ...
    , m_gradientList()
  {
    for (auto &num : m_numStyleZones) num=0;
  }
...
void State::initPens()
{
  ...
  for (float value : values) m_penSizeList.push_back(value);
}
...
MacDrawProStyleManager::MacDrawProStyleManager(MacDrawProParser &parser)
  ...
  , m_state(new MacDrawProStyleManagerInternal::State)
{
}
...
bool MacDrawProStyleManager::updateGradient(int gId, MWAWGraphicStyle &style) const
{
  ...
  auto const &gradient=m_state->m_gradientList[size_t(gId)];
  auto &finalGrad=style.m_gradient;
  finalGrad.m_type=gradient.m_type;
  finalGrad.m_stopList=gradient.m_stopList;
  finalGrad.m_angle=gradient.m_angle;
  finalGrad.m_percentCenter=gradient.m_percentCenter;
  return true;
}
...
bool MacDrawProStyleManager::readHeaderInfoStylePart(std::string &extra)
{
  ...
  for (int i=0; i<NZones; ++i) {
    m_state->m_numStyleZones[i]=static_cast<int>(input->readULong(2));
...
bool MacDrawProStyleManager::readStyles(long const(&sizeZones)[6])
{
  ...
  for (int i=0; i<numZones; ++i) {
    ...
    if (!input->checkPosition(pos + sizeZones[i])) {
      MWAW_DEBUG_MSG(("MacDrawProStyleManager::readStyles: zone %d is outside of the input\n", i));
      continue;
    }
...
bool MacDrawProStyleManager::readFontStyles(MWAWEntry const &entry)
{
  ...
  auto N=int(entry.length()/expectedSize);
  ...
  for (int j=0; j<N; ++j) {
    ...
    uint32_t flags = 0;
    auto flag=static_cast<int>(input->readULong(vers==0 ? 1 : 2));
...
bool MacDrawProStyleManager::readArrows(MWAWEntry const &entry, bool inRsrc)
{
  ...
  auto N=int(entry.length()/expectedSz);
  ...
  for (int i=0; i<N; ++i) {
    ...
    for (auto &p : pt) p=float(input->readULong(4))/65536.f;
    f << "pt=" << pt[1] << "x" << pt[0] << ",";
    if (inRsrc) {
      auto val=static_cast<int>(input->readLong(2));
      if (val!=i+1) f << "id=" << val << ",";
...
bool MacDrawProStyleManager::readDashs(MWAWEntry const &entry, bool inRsrc)
{
  ...
  for (int i=0; i<N; ++i) {
    ...
    std::vector<float> dash;
    for (int k=0; k<3; ++k) {
      auto solid=long(input->readULong(4));
      if (k==0 && inRsrc && (solid&0x8000)) // frequent
        solid &= 0x7FFF;
      auto empty=long(input->readULong(4));
...
bool MacDrawProStyleManager::readPens(MWAWEntry const &entry, bool inRsrc)
{
  ...
  for (int i=0; i<N; ++i) {
    ...
    auto penSize=long(input->readULong(4));
    if (penSize!=0x10000) f << "pen[size]=" << float(penSize)/65536.f << ",";
    if (!inRsrc) {
      int dim[2];
      for (int &j : dim) j=static_cast<int>(input->readULong(2));
      ...
      auto val=static_cast<int>(input->readLong(2)); // unit ?
...
bool MacDrawProStyleManager::readParagraphStyles(MWAWEntry const &entry)
{
  ...
  for (int j=0; j<N; ++j) {
    ...
    para.m_margins[2] = -*para.m_margins[2];// right margin is defined from right border
    double spacings[3];
    for (auto &spacing : spacings) spacing=double(input->readLong(4))/65536.;
    for (int i=0; i<3; ++i) {
      val=static_cast<int>(input->readLong(2));
    ...
    for (int i=0; i<nTabs; ++i) {
      ...
      tab.m_position=double(leftPos)+double(input->readLong(4))/65536./72;
      val=static_cast<int>(input->readLong(2));
      if (val!=0x2e) {
        int unicode= m_parserState->m_fontConverter->unicode(3, static_cast<unsigned char>(val));
        if (unicode==-1)
...
bool MacDrawProStyleManager::readRSRCZones()
{
  ...
  auto it = entryMap.lower_bound("PaDB");
  while (it != entryMap.end()) {
    if (it->first != "PaDB")
      break;
    MWAWEntry const &entry = it++->second;
...
bool MacDrawProStyleManager::readFontNames()
{
  ...
  MWAWInputStreamPtr input = m_parserState->m_rsrcParser->getInput();
  if (!input || !input->checkPosition(nameEntry.end())) {
    MWAW_DEBUG_MSG(("MacDrawProStyleManager::readFontNames: the entry seems bad\n"));
    return false;
  }
  libmwaw::DebugFile &ascFile = m_parserState->m_rsrcParser->ascii();
  libmwaw::DebugStream f;
...
bool MacDrawProStyleManager::readColors(MWAWEntry const &entry)
{
  ...
  for (int i=0; i<N; ++i) {
    ...
    val =static_cast<int>(input->readULong(2)); // index of ?
    f << "id0=" << val << ",";
    unsigned char col[3];
    for (auto &c : col) c = static_cast<unsigned char>(input->readULong(2)>>8);
...
bool MacDrawProStyleManager::readColorPatterns(MWAWEntry const &entry)
{
  ...
  auto numColors=static_cast<int>(m_state->m_colorList.size());
  for (int i=0; i<N; ++i) {
    pos=input->tell();
    ...
    for (int r=0; r<8; ++r) {
      for (int &index : indices) {
        ...
        sumColors[0]+=static_cast<int>(col.getRed());
        sumColors[1]+=static_cast<int>(col.getGreen());
        sumColors[2]+=static_cast<int>(col.getBlue());
      }
    ...
    if (bitmap.getBinary(picture)) {
      pat=MWAWGraphicStyle::Pattern
          (MWAWVec2i(8,8), picture,MWAWColor(static_cast<unsigned char>(sumColors[0]/64),static_cast<unsigned char>(sumColors[1]/64),static_cast<unsigned char>(sumColors[2]/64)));
    }
    else {
...
bool MacDrawProStyleManager::readViews(MWAWEntry const &entry)
{
  ...
  ascFile.addPos(pos-4);
  ascFile.addNote(f.str().c_str());
  auto N=int(entry.length()/dataSize);
  input->seek(pos, librevenge::RVNG_SEEK_SET);
  for (int i=0; i<N; ++i) {
    pos=input->tell();
    f.str("");
...
bool MacDrawProStyleManager::readPaletteDef(MWAWEntry const &entry)
{
  ...
  MWAWInputStreamPtr input = m_parserState->m_rsrcParser->getInput();
  auto &entryMap = m_parserState->m_rsrcParser->getEntriesMap();
  libmwaw::DebugFile &ascFile = m_parserState->m_rsrcParser->ascii();
  libmwaw::DebugStream f;
  ...
  val=static_cast<int>(input->readULong(4)); // another big number
  if (val)
...
bool MacDrawProStyleManager::readPatternPalette(MWAWEntry const &entry, int fieldSize)
{
  ...
  for (int i=0; i<N; ++i) {
    ...
    auto type=static_cast<int>(input->readULong(2)); // find 20 for 0 and 1 type
...
bool MacDrawProStyleManager::readGradientMap(MWAWEntry const &entry, int N, int fieldSize)
{
  ...
  for (int i=0; i<N; ++i) {
    ...
    case 0:
      ...
      if (decal>0.5f-1e-3f && decal < 0.5f+1e-3f) {
        decal=0;
        gradient.m_type = MWAWGraphicStyle::Gradient::G_Axial;
      }
      f << "decal=" << decal << ",";
      break;
    case 1:
      ...
      MWAW_FALLTHROUGH;
    case 2: {
      if (type==2) gradient.m_type = MWAWGraphicStyle::Gradient::G_Rectangular;
      int dim[4]; // square which defined the center rectangle
      for (int &j : dim) j=static_cast<int>(input->readULong(1));
    ...
    auto which=static_cast<int>(input->readULong(2));
    ...
    if ((type==1 || type==2) || decal>1.f-1e-3f) { // reverse color
      for (size_t j=0; j<numColors/2; ++j)
        std::swap(listColor[j],listColor[numColors-1-j]);
      decal=0;
    }
    if (decal>1e-3f && numColors>1) {
      ...
      for (size_t j=numColors-1; j>0; --j, gradPos+=step)
        gradient.m_stopList.push_back(MWAWGraphicStyle::Gradient::Stop(gradPos, listColor[j]));
    }
    float step=numColors>1 ? (1.f-decal)/float(numColors-1) : (1.f-decal);
    float gradPos=decal;
    for (auto const &color : listColor) {
      gradient.m_stopList.push_back(MWAWGraphicStyle::Gradient::Stop(gradPos, color));
      gradPos+=step;
    }
...
bool MacDrawProStyleManager::readListNames(char const *type)
{
  ...
  auto N=static_cast<int>(input->readULong(2));
  f << "N=" << N << ",";
  f << "rsId[lAttr]=" << std::hex << input->readULong(4) << std::dec << ","; // last field in the RSRCMap's mainType
  ascFile.addPos(pos-4);
  ascFile.addNote(f.str().c_str());
...
bool MacDrawProStyleManager::readPreferences1(MWAWEntry const &entry)
{
  ...
  input->seek(pos, librevenge::RVNG_SEEK_SET);
  auto val=static_cast<int>(input->readLong(2)); // prefId?
